...
export default class Watch extends THREE.Group {
        ...
        */
        ...
        for (let i = 0; i < 60; i++) {
            if (i % 5 == 0) {
                ...
                let x1 = radius2 * Math.cos(angle);
                let y1 = radius2 * Math.sin(angle);
                points.push(new THREE.Vector2(x1, y1));

                angle += increment;
            } else {
                let x0 = radius1 * Math.cos(angle);
                let y0 = radius1 * Math.sin(angle);
                points.push(new THREE.Vector2(x0, y0));
    ...
    update() {
        ...
        angle = Math.PI / 2.0 - 2.0 * Math.PI * (time[0] + time[1] / 60.0 + time[2] / 3600.0) / 12.0;
